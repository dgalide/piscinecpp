...
FragTrap::FragTrap(std::string name): name(name) {
    std::cout << name << " has born." << " Handsome Jack !" << std::endl;
...
FragTrap::FragTrap(FragTrap const &ref) {
    std::cout << ref.getName() << "has been constructor copied." << " Handsome Jack !" << std::endl;
...
FragTrap &FragTrap::operator=(FragTrap const &ref) {
    std::cout << ref.getName() << "has been operator copied." << " Handsome Jack !" << std::endl;
    if (this != &ref) {
        this->setHitPoint(ref.getHitPoint());
        this->setMaxHitPoint(ref.getMaxHitPoint());
        this->setEnergyPoint(ref.getEnergyPoint());
...
void FragTrap::meleeAttack(std::string const & target) {
    std::cout << this->getName() << " Attacks " << target << " at melee, causing " << this->getRangedAttackDamage() << " damage !" << std::endl;
}

void FragTrap::rangedAttack(std::string const & target) {
    std::cout << this->getName() << " Attacks " << target << " at range, causing " << this->getRangedAttackDamage() << " damage !" << std::endl; 
}

void        FragTrap::takeDamage(unsigned int amount) {
    std::cout << this->getName() << " beeing attacked ! Causing the pert of " << amount << " hitpoints." << std::endl;
...
void        FragTrap::beRepaired(unsigned int amount) {
    std::cout << this->getName() << " beeing repaired ! Causing the gain of " << amount << " hitpoints." << std::endl;
...
void        FragTrap::vaulthunter_dot_exe(std::string const &target) {
    if (this->getEnergyPoint() < 25) {
        std::cout << this->getName() << " does not have enough energy point :(" << std::endl;
    } else {
